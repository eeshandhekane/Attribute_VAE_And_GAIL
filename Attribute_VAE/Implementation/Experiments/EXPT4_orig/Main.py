...
for an_iter in range(TRAINING_ITR) :
	...
	if an_iter % RECORDING_ITR == 0 :
		...
		if an_iter % VALIDATION_ITR == 0 :
			...
			cap_gen_list_val, cap_gen_contrib_list_val, gen_net_val = sess.run([cap_gen_list, cap_gen_contrib_list, gen_net], feed_dict = feed_dict)
			...
			for i in range(VAL_BATCH_SIZE) :
				...
				index += 1
				cv2.imwrite('temp' + str(index).zfill(5) + '.jpg', np.reshape(gen_net_val[i], [28, 28, 1])*255.0)
				index += 1
